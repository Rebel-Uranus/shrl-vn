...
from models.model_io import ModelOptions, ModelInput, ModelOutput
import copy
        ...
        for k in scalars:
            if k != 'tools' and 'low' not in k:
                if k not in self._sums:
                    ...
                    self._sums[k] += scalars[k]
                    self._counts[k] += 1
            elif k != 'tools' and 'low' in k:
                if len(scalars[k])>0:
                    if k not in self._sums:
                        self._sums[k] = sum(scalars[k])
                        self._counts[k] = len(scalars[k])
                    else:
                        self._sums[k] += sum(scalars[k])
                        self._counts[k] += len(scalars[k])
